...
def embed_with_gemini(texts: list[str]) -> list[list[float]]:
    ...
    for i in range(0, len(texts), BATCH_SIZE):
        ...
        if i + BATCH_SIZE < len(texts):
            time.sleep(35.0)  # Delay to avoid 100 requests per minute limit
    return vectors
...
def main():
    ...
    if not os.environ.get("GEMINI_API_KEY"):
        print(
            "ERROR: GEMINI_API_KEY is not set. A Gemini API key is required to generate "
            "Korean-capable embeddings (text-embedding-004). English-only fallback is disabled "
